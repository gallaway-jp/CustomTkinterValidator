...
def main() -> None:
    ...
    ux_count = len(report.get("ux_issues", []))
    consistency_count = len(report.get("consistency_issues", []))
    rule_count = len(report.get("rule_violations", []))
    ...
    print(f"  UX issues:             {ux_count}")
    print(f"  Consistency issues:    {consistency_count}")
    print(f"  Rule violations:       {rule_count}")
